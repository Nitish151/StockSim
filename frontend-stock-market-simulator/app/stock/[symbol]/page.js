...
import { TrendingUp, TrendingDown, Clock, Star, StarOff, Info, ArrowUpRight, DollarSign, BarChart2 } from "lucide-react";
...
const StockDetailPage = ({ params }) => {
  ...
  const fetchStockData = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(`http://localhost:8080/api/stocks/${stockSymbol}`);
  ...
  const fetchChartData = async (range) => {
    try {
      const response = await fetch(`http://localhost:8080/api/stocks/${stockSymbol}/chart?range=${range}`);
      const data = await response.json();
      setChartData(data);
    } catch (error) {
  ...
  const fetchStockNews = async () => {
    try {
      const response = await fetch(`http://localhost:8080/api/stocks/${stockSymbol}/news`);
      const data = await response.json();
      setNewsItems(data);
    } catch (error) {
  ...
  const checkWatchlistStatus = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`http://localhost:8080/api/watchlist/check/${stockSymbol}`, {
  ...
  const handleTradeSubmit = async (e) => {
    ...
    try {
      ...
      alert(`${tradeType.toUpperCase()} order for ${quantity} shares of ${stockSymbol} placed successfully!`);
      setQuantity(1);
      setLimitPrice("");
    } catch (error) {
  ...
  const mockStockData = {
    symbol: stockSymbol,
    name: stockSymbol === "AAPL" ? "Apple Inc." : (stockSymbol === "MSFT" ? "Microsoft Corporation" : "Google Inc."),
    price: 239.51,
    change: 2.34,
    changePercent: 0.99,
    open: 237.17,
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg: mt-16">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          ...
          <div className="mt-4 md:mt-0">
            <p className="text-3xl font-bold">${stockData?.regularMarketPrice?.toFixed(2)}</p>
            <div className="flex items-center">
              ...
              <span className={stockData?.regularMarketChange >= 0 ? "text-green-600" : "text-red-600"}>
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                  <div>
                    ...
                    <p className="font-medium">${stockData?.regularMarketOpen?.toFixed(2)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">High</p>
                    <p className="font-medium">${stockData?.regularMarketDayHigh?.toFixed(2)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Low</p>
                    <p className="font-medium">${stockData?.regularMarketDayLow?.toFixed(2)}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-medium">{stockData?.trailingPE?.toFixed(2)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">EPS Trailing 12 months</p>
                    <p className="font-medium">${stockData?.epsTrailingTwelveMonths?.toFixed(2)}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-medium">${stockData?.fiftyTwoWeekHigh?.toFixed(2)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">52W Low</p>
                    <p className="font-medium">${stockData?.fiftyTwoWeekLow?.toFixed(2)}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <ul className="space-y-4">
                  {newsItems.map((news) => (
                    <li key={news.id} className="border-b pb-4 last:border-0 last:pb-0">
                      <a 
                        href={news.url} 
                        target="_blank" 
                      ...
                      >
                        <h3 className="font-medium text-blue-600 hover:text-blue-800 mb-1">{news.title}</h3>
                ...
                <form onSubmit={handleTradeSubmit} className="space-y-4">
                  ...
                  <div className="py-2 border-t border-b">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-lg">${calculateOrderTotal()}</span>
                    </div>
                  </div>
                  ...
                  {!isLoggedIn && (
                    <p className="text-sm text-center text-gray-500">
                      <a href="/login" className="text-blue-600 hover:underline">Log in</a> to place trades
                    </p>
                  )}
                </form>
              </CardContent>
                ...
                <div className="space-y-4">
                  ...
                  <div className="pt-4 border-t">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <p className="font-bold">${(stockData?.price * 1.15).toFixed(2)}</p>
                    </div>
                    <div className="text-sm text-gray-500">
                      <span className="font-medium text-green-600">+15%</span> upside potential
                    </div>
                  </div>
                </div>
              </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                <ul className="space-y-3">
                  {['MSFT', 'GOOG', 'AMZN', 'META'].filter(sym => sym !== stockSymbol).slice(0, 3).map((symbol) => (
                    <li key={symbol} className="border rounded-md p-3">
                      ...
                      >
                        ...
                        <div className="text-right">
                          ...
                          <p className={Math.random() > 0.5 ? "text-green-600 text-sm" : "text-red-600 text-sm"}>
